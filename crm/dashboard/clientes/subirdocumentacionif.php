...
$pathINEf  = '../../archivos/familiares/'.$id.'/inef';
...
$pathINEd  = '../../archivos/familiares/'.$id.'/ined';
...
$pathCD  = '../../archivos/familiares/'.$id.'/comprobantedomicilio';
...
$pathRC  = '../../archivos/familiares/'.$id.'/mercantil';
...
$resDocumentacionAsesor = $serviciosReferencias->traerDocumentacionPorFamiliarDocumentacion($id, $iddocumentacion);
...
$resDocumentaciones = $serviciosReferencias->traerDocumentacionPorFamiliarDocumentacionCompleta($id);
						...
						<div class="body">
							<form action="subirase.php" id="frmFileUpload" class="dropzone" method="post" enctype="multipart/form-data">
								...
								<div class="fallback">
									...
									<input type="hidden" id="idfamiliar" name="idfamiliar" value="<?php echo $id; ?>" />
								</div>
							</form>
						</div>
					</div>
...
<script>
	...
	$(document).ready(function(){
		...
		$('.btnDocumentacion').click(function() {
			...
			url = "subirdocumentacionif.php?id=<?php echo $id; ?>&documentacion=" + idTable;
			$(location).attr('href',url);
		});

		$('.btnModificar').click(function() {
			modificarFamiliarUnicaDocumentacion($('#<?php echo $campo; ?>').val(),'<?php echo $campo; ?>');
		});

		<?php if (isset($campo2)) { ?>
		$('.btnModificar2').click(function() {
			modificarFamiliarUnicaDocumentacion($('#<?php echo $campo2; ?>').val(),'<?php echo $campo2; ?>');
		});
		<?php } ?>
		...
		function modificarFamiliarUnicaDocumentacion(valor, campo) {
			$.ajax({
				url: '../../ajax/ajax.php',
				type: 'POST',
				// Form data
				...
				data: {
					accion: 'modificarFamiliarUnicaDocumentacion',
		...
		$('.guardarEstado').click(function() {
			modificarEstadoDocumentacionFamiliares($('#refestados').val());
		});

		function modificarEstadoDocumentacionFamiliares(idestado) {
			$.ajax({
				url: '../../ajax/ajax.php',
				type: 'POST',
				// Form data
				//datos del formulario
				data: {
					accion: 'modificarEstadoDocumentacionFamiliares',
					iddocumentacionfamiliar: <?php echo $iddocumentacionasociado; ?>,
					idestado: idestado
				},
				//mientras enviamos el archivo
				beforeSend: function(){
		...
		function traerImagen(contenedorpdf, contenedor) {
			$.ajax({
				data:  {idfamiliar: <?php echo $id; ?>,
						iddocumentacion: <?php echo $iddocumentacion; ?>,
						accion: 'traerDocumentacionPorFamiliarDocumentacion'},
				url:   '../../ajax/ajax.php',
				type:  'post',
				beforeSend: function () {
		...
		Dropzone.options.frmFileUpload = {
			...
			init: function() {
				this.on("sending", function(file, xhr, formData){
					formData.append("idfamiliar", '<?php echo $id; ?>');
		...
		$('.eliminar').click(function() {
			$.ajax({
				...
				// Form data
				//datos del formulario
				data: {accion: 'eliminarDocumentacionFamiliar',idasociado: <?php echo $id; ?>, iddocumentacion: <?php echo $iddocumentacion; ?>},
				//mientras enviamos el archivo
				beforeSend: function(){
					$('.btnEliminar').hide();
				},
