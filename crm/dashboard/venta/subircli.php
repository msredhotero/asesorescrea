	...
	if (!isset($_SESSION['usua_sahilices']))
	{
		...
		$resImagen = $serviciosReferencias->traerDocumentacionPorClienteDocumentacion($id,$iddocumentacion);
		...
		if (move_uploaded_file($templocation, $imagen_subida)) {
			...
			$resEliminar = $serviciosReferencias->eliminarDocumentacionasesoresPorClienteDocumentacion($id,$iddocumentacion);

			$resInsertar = $serviciosReferencias->insertarDocumentacionclientes($id,$iddocumentacion,$newname,$type,1,date('Y-m-d H:i:s'),date('Y-m-d H:i:s'),$_SESSION['usua_sahilices'],$_SESSION['usua_sahilices']);
			...
			$url = "clientes/subirdocumentacioni.php?id=".$id."&documentacion=".$iddocumentacion;
