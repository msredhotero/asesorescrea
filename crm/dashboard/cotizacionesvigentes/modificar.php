...
if (($ordenPosible >= 3) || ($ordenPosible == 0)) {
	while ($rowD = mysql_fetch_array($documentacionesrequeridas)) {
		$cadArProd .= '<li>
			 ...
			 <div class="switch">
				  <label class="btn bg-'.($rowD['color'] == '' ? 'grey' : $rowD['color']).' btnaaa'.str_replace(' ','',$rowD['documentacion']).'" style="margin-top:-6px; margin-right:5px;">'.$rowD['estadodocumentacion'].'</label>
			 </div>
		</li>';
	}
}
...
<script>
	$(document).ready(function(){
		...
		$('.frmContexisteprimaobjetivo').hide();
