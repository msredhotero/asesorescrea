	...
	if (!isset($_SESSION['usua_sahilices']))
	{
		...
		$resImagen = $serviciosReferencias->traerDocumentacionPorAsociadotemporalDocumentacion($id,$iddocumentacion);
		...
		// desarrollo
		$dir_destino = '../../archivos/asociadostemporales/'.$id.'/'.mysql_result($resDocumentacion,0,'carpeta').'/';
		...
		if (move_uploaded_file($templocation, $imagen_subida)) {
			...
			$resEliminar = $serviciosReferencias->eliminarDocumentacionasesoresPorAsociadotemporalDocumentacion($id,$iddocumentacion);

			$resInsertar = $serviciosReferencias->insertarDocumentacionasociadostemporales($id,$iddocumentacion,$newname,$type,1,date('Y-m-d H:i:s'),date('Y-m-d H:i:s'),$_SESSION['usua_sahilices'],$_SESSION['usua_sahilices']);
			...
			$url = "asociadostemporales/subirdocumentacioni.php?id=".$id."&documentacion=".$iddocumentacion;
