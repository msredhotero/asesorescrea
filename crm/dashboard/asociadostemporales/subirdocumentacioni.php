...
if ($_SESSION['idroll_sahilices'] == 10) {
	...
	$resultado 		= 	$serviciosReferencias->traerAsociadostemporalesPorUsuario($idusuario);
} else {
	$id = $_GET['id'];
...
$id = mysql_result($resultado,0,'idasociadotemporal');
...
$path  = '../../archivos/asociadostemporales/'.$id;
...
$pathINEf  = '../../archivos/asociadostemporales/'.$id.'/inef';
...
$pathINEd  = '../../archivos/asociadostemporales/'.$id.'/ined';
...
$pathCD  = '../../archivos/asociadostemporales/'.$id.'/comprobantedomicilio';
...
$pathRC  = '../../archivos/asociadostemporales/'.$id.'/mercantil';
...
$resDocumentacionAsesor = $serviciosReferencias->traerDocumentacionPorAsociadotemporalDocumentacion($id, $iddocumentacion);
...
if (mysql_num_rows($resDocumentacionAsesor) > 0) {
	...
	$iddocumentacionasociado = mysql_result($resDocumentacionAsesor,0,'iddocumentacionasociadotemporal');
...
$resDocumentaciones = $serviciosReferencias->traerDocumentacionPorAsociadotemporalDocumentacionCompleta($id);
...
<script>
	...
	$(document).ready(function(){
		...
		$('.btnModificar').click(function() {
			modificarAsociadotemporalUnicaDocumentacion($('#<?php echo $campo; ?>').val());
		});
		...
		function modificarAsociadotemporalUnicaDocumentacion(valor) {
			$.ajax({
				url: '../../ajax/ajax.php',
				type: 'POST',
				// Form data
				...
				data: {
					accion: 'modificarAsociadotemporalUnicaDocumentacion',
					idasociadotemporal: <?php echo $id; ?>,
		...
		$('.guardarEstado').click(function() {
			modificarEstadoDocumentacionAsociadostemporales($('#refestados').val());
		});

		function modificarEstadoDocumentacionAsociadostemporales(idestado) {
			$.ajax({
				url: '../../ajax/ajax.php',
				type: 'POST',
				// Form data
				//datos del formulario
				data: {
					accion: 'modificarEstadoDocumentacionAsociadostemporales',
					iddocumentacionasociadotemporal: <?php echo $iddocumentacionasociado; ?>,
					idestado: idestado
				},
				//mientras enviamos el archivo
				beforeSend: function(){
		...
		function traerImagen(contenedorpdf, contenedor) {
			$.ajax({
				data:  {idasociadotemporal: <?php echo $id; ?>,
						iddocumentacion: <?php echo $iddocumentacion; ?>,
						accion: 'traerDocumentacionPorAsociadotemporalDocumentacion'},
				url:   '../../ajax/ajax.php',
				type:  'post',
				beforeSend: function () {
		...
		$('.eliminar').click(function() {
			$.ajax({
				...
				// Form data
				//datos del formulario
				data: {accion: 'eliminarDocumentacionAsociadotemporal',idasociadotemporal: <?php echo $id; ?>, iddocumentacion: <?php echo $iddocumentacion; ?>},
				//mientras enviamos el archivo
				beforeSend: function(){
					$('.btnEliminar').hide();
				},
