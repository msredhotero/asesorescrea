...
if (!(isset($_GET['id']))) {
	if (!(isset($_GET['token']))) {
		...
		if (mysql_num_rows($resCI) > 0) {
			...
			$id = mysql_result($resCI,0,'idreferencia');
	...
	$resCI = $serviciosComercio->traerComercioinicioPorReferencia(15, 'dbperiodicidadventasdetalle', 'idperiodicidadventadetalle', $id);
...
if (mysql_result($resCotizaciones,0,'refproductosaux') == 0) {
	...
	$resProducto = $serviciosReferencias->traerProductosPorId(mysql_result($resCotizaciones,0,'refproductos'));
} else {
...
if (mysql_num_rows($existeComercio) > 0) {
	...
	$idComercio = $serviciosComercio->insertarComercioinicio($token,$comtotal,$comcurrency,'',$comorder_id,$commerchant,$comstore,$comterm,$comdigest,$urlback,$reforigencomercio,$refestadotransaccion,$refafiliados,$fechacrea,$usuariocrea,$vigencia,$observaciones,$usuariocrea,15,$id);
		        ...
		        <div class="card">
		          ...
		          <div class="body table-responsive">
							<div class="text-center">
								<h1 class="display-4">CONFIRMA EL PAGO DE TU RECIBO</h1>
								<!-- IMAGEN DEL RECIBO -->
							</div>
			            ...
			            <p><small>Al concretar tu compra estarás aceptando nuestros Términos de servicio y Política de privacidad. </small></p>
							<p><small>Seguros Inbursa es el receptor de todos los pagos que se generan en esta plataforma, Asesores CREA es responsable de su correcta aplicación, emisión de las polizas en referencia y brindar todo tipo de atención a nuestros clientes.</small></p>
