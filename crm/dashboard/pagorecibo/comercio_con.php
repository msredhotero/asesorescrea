...
// solo para
if ((mysql_num_rows($resUsuario)>0) && (!isset($_SESSION['usua_sahilices'])) )
{
...
if ($error == 0) {
	...
	if ($reforigencomercio == 7) {
		...
		///// quinto paso ****
		$resModificarPN = $serviciosReferencias->modificarPeriodicidadventasdetalleEstado($idcotizacion,2,$_SESSION['usua_sahilices'],date('Y-m-d H:i:s'));
	...
	if ((integer)$resPP > 0) {
		$resPagoAux = $serviciosReferencias->traerPagosPorId($resPago);
		...
		///// onceavo paso ****
		$resAplicar = $serviciosReferencias->aplicarPago($resPago,'1');
	}
...
/// doceavo paso ****
// modifico el token por donde ingreso.
$resModificarToken = $serviciosReferencias->modificarTokenasesoresEstado($token,2);
