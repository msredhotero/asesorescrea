	...
	if (!isset($_SESSION['usua_sahilices']))
	{
		...
		$resImagen = $serviciosReferencias->traerDocumentacionPorAsesoresunicaDocumentacion($idpostulante,$iddocumentacion);
		...
		// desarrollo
		$dir_destino = '../../archivos/asesores/'.$idpostulante.'/'.mysql_result($resDocumentacion,0,'carpeta').'/';
		...
		if (!file_exists('../../archivos/asesores/'.$idpostulante.'/')) {
			mkdir('../../archivos/asesores/'.$idpostulante.'/', 0777);
		}
		...
		if (move_uploaded_file($templocation, $imagen_subida)) {
			...
			$resEliminar = $serviciosReferencias->eliminarDocumentacionasesoresunicaPorAsesoresunicaDocumentacion($idpostulante,$iddocumentacion);

			$resInsertar = $serviciosReferencias->insertarDocumentacionasesoresunica($idpostulante,$iddocumentacion,$newname,$type,1,date('Y-m-d H:i:s'),date('Y-m-d H:i:s'),$_SESSION['usua_sahilices'],$_SESSION['usua_sahilices']);
			...
			$url = "asesores/expediente.php?id=".$idpostulante."&documentacion=".$iddocumentacion;
