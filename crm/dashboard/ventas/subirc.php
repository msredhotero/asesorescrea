	...
	if (!isset($_SESSION['usua_sahilices']))
	{
		...
		// desarrollo
		$dir_destino = '../../archivos/cobros/'.$id.'/'.mysql_result($resDocumentacion,0,'carpeta').'/';
		...
		if (!file_exists('../../archivos/cobros/'.$id.'/')) {
			mkdir('../../archivos/cobros/'.$id.'/', 0777);
		}
		...
		if (move_uploaded_file($templocation, $imagen_subida)) {
			...
			$resInsertar = $serviciosReferencias->insertarDocumentacionventas($id,$iddocumentacion,$newname,$type,1,date('Y-m-d H:i:s'),date('Y-m-d H:i:s'),$_SESSION['usua_sahilices'],$_SESSION['usua_sahilices']);
