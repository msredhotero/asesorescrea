...
function insertarPixel($reforigen,$donde,$evento,$dato1,$dato2,$dato3,$fechacrea) {
   ...
   function insertarSolicitudvrim($reftokenvrim,$cveasesor,$tipoasesor,$cveoficina,$tipooficina,$tiposolicitud,$tipopersona,$razonsocial,$nombre,$apellidopaterno,$apellidomaterno,$fechanacimiento,$correo,$telefonofijo,$celular,$cveproducto,$tnombre,$tapellidopaterno,$tapellidomaterno,$tfechanacimiento,$tcorreo,$tcelular,$fechacrea,$usuariocrea) {
   $sql = "insert into dbsolicitudvrim(idsolicitudvrim,reftokenvrim,cveasesor,tipoasesor,cveoficina,tipooficina,tiposolicitud,tipopersona,razonsocial,nombre,apellidopaterno,apellidomaterno,fechanacimiento,correo,telefonofijo,celular,cveproducto,tnombre,tapellidopaterno,tapellidomaterno,tfechanacimiento,tcorreo,tcelular,fechacrea,usuariocrea)
   values ('',".$reftokenvrim.",'".$cveasesor."','".$tipoasesor."','".$cveoficina."','".$tipooficina."','".$tiposolicitud."','".$tipopersona."','".$razonsocial."','".$nombre."','".$apellidopaterno."','".$apellidomaterno."','".$fechanacimiento."','".$correo."','".$telefonofijo."','".$celular."','".$cveproducto."','".$tnombre."','".$tapellidopaterno."','".$tapellidomaterno."','".$tfechanacimiento."','".$tcorreo."','".$tcelular."','".$fechacrea."','".$usuariocrea."')";
   $res = $this->query($sql,1);
   return $res;
   }


   function modificarSolicitudvrim($id,$reftokenvrim,$cveasesor,$tipoasesor,$cveoficina,$tipooficina,$tiposolicitud,$tipopersona,$razonsocial,$nombre,$apellidopaterno,$apellidomaterno,$fechanacimiento,$correo,$telefonofijo,$celular,$cveproducto,$tnombre,$tapellidopaterno,$tapellidomaterno,$tfechanacimiento,$tcorreo,$tcelular,$fechacrea,$usuariocrea) {
   $sql = "update dbsolicitudvrim
   set
   reftokenvrim = ".$reftokenvrim.",cveasesor = '".$cveasesor."',tipoasesor = '".$tipoasesor."',cveoficina = '".$cveoficina."',tipooficina = '".$tipooficina."',tiposolicitud = '".$tiposolicitud."',tipopersona = '".$tipopersona."',razonsocial = '".$razonsocial."',nombre = '".$nombre."',apellidopaterno = '".$apellidopaterno."',apellidomaterno = '".$apellidomaterno."',fechanacimiento = '".$fechanacimiento."',correo = '".$correo."',telefonofijo = '".$telefonofijo."',celular = '".$celular."',cveproducto = '".$cveproducto."',tnombre = '".$tnombre."',tapellidopaterno = '".$tapellidopaterno."',tapellidomaterno = '".$tapellidomaterno."',tfechanacimiento = '".$tfechanacimiento."',tcorreo = '".$tcorreo."',tcelular = '".$tcelular."',fechacrea = '".$fechacrea."',usuariocrea = '".$usuariocrea."' where idsolicitudvrim =".$id;
   $res = $this->query($sql,0);
   return $res;
   }


   function eliminarSolicitudvrim($id) {
   $sql = "delete from dbsolicitudvrim where idsolicitudvrim =".$id;
   $res = $this->query($sql,0);
   return $res;
   }


   function traerSolicitudvrim() {
   $sql = "select
   s.idsolicitudvrim,
   s.reftokenvrim,
   s.cveasesor,
   ...
   /* Fin */
   /* Fin de la Tabla: dbsolicitudvrim */
...
function eliminarTokenvrim($id) {
$sql = "delete from dbtokenvrim where idtokenvrim =".$id;
$res = $this->query($sql,0);
return $res;
}


function traerTokenvrim() {
$sql = "select
t.idtokenvrim,
t.grant_type,
t.username,
...
function traerTokenvrimPorId($id) {
...
/* Fin */
/* /* Fin de la Tabla: dbtokenvrim*/



   function generarRecibosCalculados($id) {
      $resVenta = $this->traerVentasPorIdCompleto($id);

      $resPV = $this->traerPeriodicidadventasPorVenta($id);

      $arCad = array();

      $refperiodicidadventas = mysql_result($resPV,0,'idperiodicidadventa');

      $primaneta = mysql_result($resVenta,0,'primaneta');
      $financiamiento = mysql_result($resVenta,0,'financiamiento');
      $gastosexpedicion = mysql_result($resVenta,0,'gastosexpedicion');
      $iva = mysql_result($resVenta,0,'iva');
      $comisioncedida = -1 * mysql_result($resVenta,0,'comisioncedida');

      $meses = mysql_result($resPV,0,'meses');

      $frecuencia = 12 / $meses;

      $primanetaP = $primaneta / $meses;
      $financiamientoP = $financiamiento / $meses;
      $comisioncedidaP = $comisioncedida / $meses;
      ...
      for ($i=1;$i<=$meses;$i++) {
         ...
         $resCalculo = $this->traerUltimoMesNuevo($id,$frecuencia);

         if ($primero == 0) {
            $fechavencimiento = mysql_result($resCalculo,0,'fechavencimientoaux');
            $fechafinservicio = mysql_result($resCalculo,0,'fechafinservicioaux');

            $montototal = ($primanetaP + $financiamientoP + $gastosexpedicion + $comisioncedidaP) * (1 + $iva / 100);

            $resPVD = $this->insertarPeriodicidadventasdetalle($refperiodicidadventas,$montototal,$primanetaP,0,0,'NULL',$fechavencimiento,1,$_SESSION['usuaid_sahilices'],$_SESSION['usuaid_sahilices'],date('Y-m-d H:i:s'),date('Y-m-d H:i:s'),$this->generaNroRecibo(),'NULL',0,$fechafinservicio);
            ...
            $montototal = ($primanetaP + $financiamientoP + $comisioncedidaP) * (1 + $iva / 100);
   ...
   function emailDePagoDirecto($id) {
      $resVenta = $this->traerVentasPorIdCompleto($id);

      $resPeriodicidadDetalle = $this->traerPeriodicidadventasdetallePorVenta($id);

      $idProducto = mysql_result($resVenta,0,'refproductos');

      $resProducto = $this->traerProductosPorId($idProducto);

      $producto = mysql_result($resProducto,0,'producto');

      $reftipoproductorama = mysql_result($resProducto,0,'reftipoproductorama');

      $claveasesor = mysql_result($resVenta,0,'claveasesor');

      //solo para pagos directos, necesito el primer recibo
      if (mysql_num_rows($resPeriodicidadDetalle)>0) {

         $idperiodicidadventadetalle = mysql_result($resPeriodicidadDetalle,0,'idperiodicidadventadetalle');

         $montototal = mysql_result($resPeriodicidadDetalle,0,'montototal');
         ...
         $cuerpo .= '<h3><small><p>Link para el pago de su producto/servicio: '.$producto.' </small></h3><p>';
         ...
         $resCI = $this->traerComercioinicioPorReferencia(15, 'dbperiodicidadventasdetalle', 'idperiodicidadventadetalle', $idperiodicidadventadetalle);
         ...
         if (mysql_num_rows($resCI) > 0) {
            ...
            $idComercio = $this->insertarComercioinicio($tokenPago,$comtotal,$comcurrency,'',$comorder_id,$commerchant,$comstore,$comterm,$comdigest,$urlback,$reforigencomercio,$refestadotransaccion,$refafiliados,$fechacrea,$usuariocrea,$vigencia,$observaciones,$usuariocrea,15, $idperiodicidadventadetalle);
         ...
         if ($claveasesor != '28222') {
            $email = mysql_result($resVenta,0,'emialasesor');
         } else {
            $email = mysql_result($resVenta,0,'emailcliente');
         }

         $mensaje = 'Pago del recibo '.mysql_result($resPeriodicidadDetalle,0,'nrorecibo').' de la poliza: '.mysql_result($resVenta,0,'nropoliza');
         ...
         $token = $this->GUID();
         if ($claveasesor != '28222') {
            $resAutoLogin = $this->insertarAutologin(mysql_result($resVenta,0,'refusuariosasesor'),$token,$url,'0');
         } else {
            $resAutoLogin = $this->insertarAutologin(mysql_result($resVenta,0,'refusuarios'),$token,$url,'0');
         }


         // datos para el pago por transferencia
         $urlTransferencia = 'cobranza/subirdocumentacioni.php?id='.$idperiodicidadventadetalle.'&iddocumentacion=39';
         $tokenTransferencia = $this->GUID();

         if ($claveasesor != '28222') {
            $resAutoLoginTransferencia = $this->insertarAutologin(mysql_result($resVenta,0,'refusuariosasesor'),$tokenTransferencia,$urlTransferencia,'0');
         } else {
            $resAutoLoginTransferencia = $this->insertarAutologin(mysql_result($resVenta,0,'refusuarios'),$tokenTransferencia,$urlTransferencia,'0');
         }


         // datos para el pago por transferencia
         $urlCallCenter = 'cobranza/subirdocumentacionfolio.php?id='.$idperiodicidadventadetalle;
         $tokenCallCenter = $this->GUID();

         if ($claveasesor != '28222') {
            $resAutoLoginCallCenter = $this->insertarAutologin(mysql_result($resVenta,0,'refusuariosasesor'),$tokenCallCenter,$urlCallCenter,'0');
         } else {
            $resAutoLoginCallCenter = $this->insertarAutologin(mysql_result($resVenta,0,'refusuarios'),$tokenCallCenter,$urlCallCenter,'0');
         }
         ...
         $cuerpoAux .= '<h4>Haga click <a href="https://asesorescrea.com/desarrollo/crm/alogin.php?token='.$tokenTransferencia.'">AQUI</a> para acceder y adjuntar el comprobante de pago, nuestra administración, validara estos datos cargados.</h4><h5>Clabe Interbancaria para transferencias: 036180500079200351</h5>';

         $cuerpoAux .= '<h4>Haga click <a href="https://asesorescrea.com/desarrollo/crm/alogin.php?token='.$tokenTransferencia.'">AQUI</a> para acceder y adjuntar el comprobante de pago, si pago por sucursal de inbursa</h5>';

         $cuerpoAux .= '<h4>Haga click <a href="https://asesorescrea.com/desarrollo/crm/alogin.php?token='.$tokenCallCenter.'">AQUI</a> para acceder y cargar el folio</h5>';
         ...
         //$resI = $this->insertarAlertas($reftipoalerta,15, $idperiodicidadventadetalle,$email,$mensaje,'1');

         $gestor = fopen('logemails'.date('Y_m_d_H_i_s').'_email_'.$email.'_id_'.$id.'_idpp_'.$idperiodicidadventadetalle.'.txt', 'w');
         fwrite($gestor, $cuerpo.$cuerpoAux.' '.date('Y-m-d H:i:s'));
         fclose($gestor);
      }
   ...
   function eliminarCotizacionesdirectorioPorCotizacionArea($id,$idarea) {
      $sql = "DELETE cd from dbcotizacionesdirectorio cd
                    INNER JOIN
         ...
         where cd.refcotizaciones =".$id." and a.idareadirectorio=".$idarea;
      $res = $this->query($sql,0);
      return $res;
   }
   ...
   function traerCotizacionesdirectorioPorCotizacionArea($id,$idarea) {
      $sql = "select
         cd.idcotizaciondirectorio,
         cd.refcotizaciones,
         cd.refdirectorioasesores,
         da.razonsocial
         from dbcotizacionesdirectorio cd
                    INNER JOIN
                dbcotizaciones c ON c.idcotizacion = cd.refcotizaciones
                    INNER JOIN
                dbdirectorioasesores da ON da.iddirectorioasesor = cd.refdirectorioasesores
...
/* /* Fin de la Tabla: dbtransferenciarecibos*/
   ...
   function enviarEmailModificacionCotizacion($id,$dato,$adcional,$enviaEmail=1,$refevento) {
      ...
      if (mysql_result($resEvento,0,'enviaemailagestion') == '1') {
         ...
         while ($rowC = mysql_fetch_array($resContactos)) {
      ...
      if (mysql_result($resEvento,0,'enviaemailaagente') == '1') {
         ...
         $gestor = fopen('logemails'.date('Y_m_d_H_i_s').'_agente_etapa_'.$refetapa.'_id_'.$id.'_evento_'.$refevento.'.txt', 'w');
...
/* /* Fin de la Tabla: tbtipoalerta*/
   ...
   */
   ...
   function insertarRenovaciones($refventas,$nuevaventa,$fechacreacion,$refcotizaciones,$nuevacotizacion) {
   $sql = "insert into dbrenovaciones(idrenovacion,refventas,nuevaventa,fechacreacion,refcotizaciones,nuevacotizacion)
   values ('',".$refventas.",".$nuevaventa.",'".$fechacreacion."',".$refcotizaciones.",".$nuevacotizacion.")";
   $res = $this->query($sql,1);
   return $res;
   }


   function modificarRenovaciones($id,$refventas,$nuevaventa,$fechacreacion,$refcotizaciones,$nuevacotizacion) {
   $sql = "update dbrenovaciones
   set
   refventas = ".$refventas.",nuevaventa = ".$nuevaventa.",fechacreacion = '".$fechacreacion."',refcotizaciones = ".$refcotizaciones.",nuevacotizacion = ".$nuevacotizacion."
   where idrenovacion =".$id;
   $res = $this->query($sql,0);
   return $res;
   }

   function modificarRenovacionesPorVenta($id,$nuevaventa) {
      $sql = "update dbrenovaciones
      set
   ...
   function traerRenovacionesPorId($id) {
      $sql = "select idrenovacion,refventas,nuevaventa,fechacreacion,refcotizaciones,nuevacotizacion from dbrenovaciones where idrenovacion =".$id;
      $res = $this->query($sql,0);
      return $res;
   }

   function traerRenovacionesPorVentaVieja($id) {
      $sql = "select idrenovacion,refventas,nuevaventa,fechacreacion,refcotizaciones,nuevacotizacion from dbrenovaciones where refventas =".$id;
      $res = $this->query($sql,0);
      return $res;
   }

   function traerRenovacionesPorVentaNueva($id) {
      $sql = "select idrenovacion,refventas,nuevaventa,fechacreacion,refcotizaciones,nuevacotizacion from dbrenovaciones where nuevaventa =".$id;
      $res = $this->query($sql,0);
      return $res;
   }

   function traerRenovacionesPorCotizacionVieja($id) {
      $sql = "select idrenovacion,refventas,nuevaventa,fechacreacion,refcotizaciones,nuevacotizacion from dbrenovaciones where refcotizaciones =".$id;
      $res = $this->query($sql,0);
      return $res;
   }

   function traerRenovacionesPorCotizacionNueva($id) {
      $sql = "select idrenovacion,refventas,nuevaventa,fechacreacion,refcotizaciones,nuevacotizacion from dbrenovaciones where nuevacotizacion =".$id;
      $res = $this->query($sql,0);
      return $res;
   }
...
/* /* Fin de la Tabla: tbprocesocotizacion*/
               ...
               if ($row['idtiporespuesta'] == 1) {
                  ...
                  if ($row['obligatoria'] == '1') {
                     $cadInput .= '<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 frmContrespuesta" style="display:block">
                        ...
                        <div class="form-group input-group">
                           <div class="form-line">
                              <input type="text" autocomplete="off" class="form-control" id="respuesta'.str_replace(')','',str_replace('(','',str_replace('.','',str_replace(':','',str_replace(' ','',$row['pregunta']))))).'" name="respuesta'.$row['idpreguntacuestionario'].'" required="" aria-required="true" aria-invalid="false" value="'.$cadValorRespuesta.'" >
                     ...
                     $cadInput .= '<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 frmContrespuesta" style="display:block">
                        ...
                        <div class="form-group input-group">
                           <div class="form-line">
                              <input type="text" autocomplete="off" class="form-control" id="respuesta'.str_replace(')','',str_replace('(','',str_replace('.','',str_replace(':','',str_replace(' ','',$row['pregunta']))))).'" value="'.$cadValorRespuesta.'" name="respuesta'.$row['idpreguntacuestionario'].'" >
   ...
   function generaNroRecibo() {
	...
	function insertarPeriodicidadventasdetalle($refperiodicidadventas,$montototal,$primaneta,$porcentajecomision,$montocomision,$fechapago,$fechavencimiento,$refestadopago,$usuariocrea,$usuariomodi,$fechacrea,$fechamodi,$nrorecibo,$fechapagoreal='',$refformapago=0,$fechafinservicio) {
      if ($fechapagoreal == '') {
         $sql = "insert into dbperiodicidadventasdetalle(idperiodicidadventadetalle,refperiodicidadventas,montototal,primaneta,porcentajecomision,montocomision,fechapago,fechavencimiento,refestadopago,usuariocrea,usuariomodi,fechacrea,fechamodi,nrorecibo,fechapagoreal,refformapago,fechafinservicio)
   		values ('',".$refperiodicidadventas.",".$montototal.",".$primaneta.",".$porcentajecomision.",".$montocomision.",'".$fechapago."','".$fechavencimiento."',".$refestadopago.",'".$usuariocrea."','".$usuariomodi."','".$fechacrea."','".$fechamodi."','".$nrorecibo."',NULL,".$refformapago.",'".$fechafinservicio."')";
      } else {
         $sql = "insert into dbperiodicidadventasdetalle(idperiodicidadventadetalle,refperiodicidadventas,montototal,primaneta,porcentajecomision,montocomision,fechapago,fechavencimiento,refestadopago,usuariocrea,usuariomodi,fechacrea,fechamodi,nrorecibo,fechapagoreal,refformapago,fechafinservicio)
   		values ('',".$refperiodicidadventas.",".$montototal.",".$primaneta.",".$porcentajecomision.",".$montocomision.",'".$fechapago."','".$fechavencimiento."',".$refestadopago.",'".$usuariocrea."','".$usuariomodi."','".$fechacrea."','".$fechamodi."','".$nrorecibo."','".$fechapagoreal."',".$refformapago.",'".$fechafinservicio."')";
      }
	...
	function modificarPeriodicidadventasdetalle($id,$refperiodicidadventas,$montototal,$primaneta,$porcentajecomision,$montocomision,$fechapago,$fechavencimiento,$refestadopago,$usuariomodi,$fechamodi,$nrorecibo,$fechapagoreal,$refformapago=0,$fechafinservicio) {
		$sql = "update dbperiodicidadventasdetalle
		set
		refperiodicidadventas = ".$refperiodicidadventas.",montototal = ".$montototal.",primaneta = ".$primaneta.",porcentajecomision = ".$porcentajecomision.",montocomision = ".$montocomision.",fechapago = '".$fechapago."',fechavencimiento = '".$fechavencimiento."',refestadopago = ".$refestadopago.",usuariomodi = '".$usuariomodi."',fechamodi = '".$fechamodi."',nrorecibo = '".$nrorecibo."',fechapagoreal = '".$fechapagoreal."',refformapago=".$refformapago.",fechafinservicio = '".$fechafinservicio."' where idperiodicidadventadetalle =".$id;
		$res = $this->query($sql,0);
		return $res;
	}
   ...
   function eliminarPeriodicidadventasdetalleEndoso($id,$version) {
	...
	function traerPeriodicidadventasdetallePorId($id) {
		$sql = "select idperiodicidadventadetalle,refperiodicidadventas,montototal,primaneta,porcentajecomision,montocomision,fechapago,fechavencimiento,refestadopago,usuariocrea,usuariomodi,fechacrea,fechamodi,nrorecibo,fechapagoreal,refformapago,fechafinservicio from dbperiodicidadventasdetalle where idperiodicidadventadetalle =".$id;
		$res = $this->query($sql,0);
		return $res;
	}
   ...
   function traerVentaUnicaDocumentacion($campo,$id) {
	...
	function traerPeriodicidadventasdetallePorIdCompleto($id) {
      ...
      cli.refusuarios , ase.refusuarios as refusuariosasesor, usu.email, usua.email as emailasesor, co.refproductos, pvd.fechafinservicio
	...
	function traerPeriodicidadventasdetallePorVenta($id) {
      ...
      pv.reftipocobranza, pd.fechapagoreal, pd.refformapago , fp.formapago, pd.fechafinservicio
   ...
   function traerPeriodicidadventasdetallePorVentaPagados($idnuevo) {
		...
		pd.fechapago,pd.fechavencimiento,pd.refestadopago,
		pd.usuariocrea,pd.usuariomodi,pd.fechacrea,pd.fechamodi, pd.nrorecibo,
      pv.reftipocobranza , pd.fechapagoreal, pd.fechafinservicio
		from dbperiodicidadventasdetalle pd
		inner join dbperiodicidadventas pv ON pv.idperiodicidadventa = pd.refperiodicidadventas
		inner join dbventas v on v.idventa = pv.refventas
	...
	function traerUltimoMes($id) {
      ...
      max(v.vigenciadesde) as fechafinservicio
		from dbperiodicidadventasdetalle pd
		inner join dbperiodicidadventas pv ON pv.idperiodicidadventa = pd.refperiodicidadventas
		inner join dbventas v on v.idventa = pv.refventas
		where v.idventa =".$id;
   ...
   function traerUltimoMesNuevo($id,$meses) {
		$sql = "select
            r.fechafinservicio,
            DATE_ADD(r.fechafinservicio, INTERVAL 30 DAY) as fechavencimiento,
            r.montototal,
            r.primaneta,
            r.porcentajecomision,
            r.montocomision,
            r.fechafinservicioaux,
            r.fechavencimientoaux
            from	(
            	select
         			DATE_ADD(max(pd.fechafinservicio), INTERVAL ".$meses." MONTH) as fechafinservicio,
         			max(pd.montototal) as montototal,
         			max(pd.primaneta) as primaneta,
         			max(pd.porcentajecomision) as porcentajecomision,
         			max(pd.montocomision) as montocomision,
         			max(v.vigenciadesde) as fechafinservicioaux,
         			DATE_ADD(max(v.vigenciadesde), INTERVAL 30 DAY) as fechavencimientoaux
         			from dbventas v
         			left join dbperiodicidadventas pv ON pv.refventas= v.idventa
         			left join dbperiodicidadventasdetalle pd on pv.idperiodicidadventa = pd.refperiodicidadventas
         			where v.idventa =".$id."
		) r";
		$res = $this->query($sql,0);
		return $res;
   ...
   function insertarPeriodicidadventasPorVenta($refventas,$idventa) {
		$sql = "insert into dbperiodicidadventas(idperiodicidadventa,refventas,reftipoperiodicidad,reftipocobranza,banco,afiliacionnumber,tipotarjeta,vigencia,tarjetanumber)
		select '',".$refventas.",reftipoperiodicidad,reftipocobranza,banco,afiliacionnumber,tipotarjeta,vigencia,tarjetanumber from dbperiodicidadventas where refventas =".$idventa;
		$res = $this->query($sql,1);
		return $res;
	}

	function insertarPeriodicidadventas($refventas,$reftipoperiodicidad,$reftipocobranza,$banco,$afiliacionnumber,$tipotarjeta,$vigencia,$tarjetanumber) {
		$sql = "insert into dbperiodicidadventas(idperiodicidadventa,refventas,reftipoperiodicidad,reftipocobranza,banco,afiliacionnumber,tipotarjeta,vigencia,tarjetanumber)
		values ('',".$refventas.",".$reftipoperiodicidad.",".$reftipocobranza.",'".$banco."','".$afiliacionnumber."','".$tipotarjeta."','".$vigencia."','".$tarjetanumber."')";
		$res = $this->query($sql,1);
		return $res;
	}

   //pendiente 14/04/2021
   function insertarPeriodicidadventasPorMetodoPago($refventas,$refmetodopago) {
	...
	function modificarPeriodicidadventas($id,$refventas,$reftipoperiodicidad,$reftipocobranza,$vigencia,$tarjetanumber) {
		$sql = "update dbperiodicidadventas
		set
		refventas = ".$refventas.",reftipoperiodicidad = ".$reftipoperiodicidad.",reftipocobranza = ".$reftipocobranza.",vigencia = ".$vigencia.",tarjetanumber = ".$tarjetanumber."
   ...
   function eliminarPeriodicidadventasPorVentas($id) {
	...
	function traerPeriodicidadventasPorId($id) {
		$sql = "select idperiodicidadventa,refventas,reftipoperiodicidad,reftipocobranza,banco,afiliacionnumber,tipotarjeta,vigencia,tartarjetanumber from dbperiodicidadventas where idperiodicidadventa =".$id;
		$res = $this->query($sql,0);
		return $res;
	}
   ...
   function traerVentasPorVersionVenta($version,$idventa) {
	...
	function insertarVentas($refcotizaciones,$refestadoventa,$primaneta,$primatotal,$fechavencimientopoliza,$nropoliza,$fechacrea,$fechamodi,$usuariocrea,$usuariomodi,$foliotys,$foliointerno,$refproductosaux,$refventas,$version,$observaciones='',$vigenciadesde,$fechaemision='',$reftipomoneda=1,$comisioncedida,$financiamiento,$gastosexpedicion,$iva) {

      if ($fechaemision == '') {
         $sql = "insert into dbventas(idventa,refcotizaciones,refestadoventa,primaneta,primatotal,fechavencimientopoliza,nropoliza,fechacrea,fechamodi,usuariocrea,usuariomodi,foliotys,foliointerno,refproductosaux, refventas, version, observaciones,vigenciadesde,fechaemision,reftipomoneda,comisioncedida,financiamiento,gastosexpedicion,iva)
   		values ('',".$refcotizaciones.",".$refestadoventa.",".$primaneta.",".$primatotal.",'".$fechavencimientopoliza."','".$nropoliza."','".$fechacrea."','".$fechamodi."','".$usuariocrea."','".$usuariomodi."','".$foliotys."','".$foliointerno."',".$refproductosaux.",".$refventas.",".$version.",'".$observaciones."','".$vigenciadesde."',NULL,".$reftipomoneda.",".$comisioncedida.",".$financiamiento.",".$gastosexpedicion.",".$iva.")";
      } else {
         $sql = "insert into dbventas(idventa,refcotizaciones,refestadoventa,primaneta,primatotal,fechavencimientopoliza,nropoliza,fechacrea,fechamodi,usuariocrea,usuariomodi,foliotys,foliointerno,refproductosaux, refventas, version, observaciones,vigenciadesde,fechaemision,reftipomoneda,comisioncedida,financiamiento,gastosexpedicion,iva)
   		values ('',".$refcotizaciones.",".$refestadoventa.",".$primaneta.",".$primatotal.",'".$fechavencimientopoliza."','".$nropoliza."','".$fechacrea."','".$fechamodi."','".$usuariocrea."','".$usuariomodi."','".$foliotys."','".$foliointerno."',".$refproductosaux.",".$refventas.",".$version.",'".$observaciones."','".$vigenciadesde."','".$fechaemision."',".$reftipomoneda.",".$comisioncedida.",".$financiamiento.",".$gastosexpedicion.",".$iva.")";
      }
   ...
   function traerVentaEstadosajax($length, $start, $busqueda,$colSort,$colSortDir,$responsableComercial,$min,$max,$agente,$estado,$vencidas) {
		$where = '';
      $cadEstado = ' v.refestadoventa in ('.$estado.') ';
		$roles = '';

      if ($vencidas == 1) {
         $cadVencidas = ' and v.fechavencimientopoliza < current_date() ';
      } else {
         $cadVencidas = ' and v.fechavencimientopoliza >= current_date() ';
      }
      ...
      inner join dbventas v on v.refcotizaciones = vf.refcotizaciones and v.version = vf.version and v.refproductosaux = vf.refproductosaux
          ...
          tbtipoperiodicidad tp ON tp.idtipoperiodicidad = pv.reftipoperiodicidad
            inner join tbtipocobranza tcc on tcc.idtipocobranza = pv.reftipocobranza
              left JOIN
          dbperiodicidadventasdetalle pvd ON pvd.refperiodicidadventas = pv.idperiodicidadventa
      where ".$cadEstado.$cadVencidas." ".$where.$cadFecha.$cadAgente."
   ...
   function traerRenovacionesPorVencer($idasesor, $idcliente, $idproducto, $idcotizacion) {
      $sql = "select
		v.idventa,
		(case when cli.reftipopersonas = '1' then concat(cli.apellidopaterno, ' ', cli.apellidomaterno, ' ', cli.nombre) else cli.razonsocial end) as cliente,
		(case when v.refproductosaux > 0 then proa.producto else pro.producto end) as producto,
      ...
      left join tbproductos proa ON proa.idproducto = vf.refproductosaux
      inner join dbventas v on v.refcotizaciones = vf.refcotizaciones and v.version = vf.version and v.refproductosaux = vf.refproductosaux
      left join dbrenovaciones rn ON rn.refventas = v.idventa and rn.refcotizaciones = ".$idcotizacion."

		where v.refestadoventa = 1
            and rn.idrenovacion is null
            and DATEDIFF(v.fechavencimientopoliza,CURDATE()) <= 60
            and ase.idasesor = ".$idasesor."
            and pro.idproducto = ".$idproducto."
   ...
   function traerVentasRenovacionesPendientesajax($length, $start, $busqueda,$colSort,$colSortDir,$responsableComercial,$idasesor) {
      ...
      if ($idasesor != '') {
		...
		v.fechavencimientopoliza,
		v.nropoliza,
      DATEDIFF(CURDATE(),v.fechavencimientopoliza) as dias,
		est.estadocotizacion,
		v.foliointerno,
		c.refclientes,
		c.refproductos,
      ...
      left join dbrenovaciones rn ON rn.refventas = v.idventa
		...
		where v.refestadoventa = 1 and rn.idrenovacion is null and DATEDIFF(v.fechavencimientopoliza,CURDATE()) <= 60 ".$where.$cadAsesor."
   ...
   function traerVentasRenovacionesajax($length, $start, $busqueda,$colSort,$colSortDir,$responsableComercial,$refusuario='',$idasesor) {
      ...
      // ver con javier esto: v.vigenciadesde > CURDATE()
   ...
   function traerVentasHistoricoajax($length, $start, $busqueda,$colSort,$colSortDir,$responsableComercial,$min,$max,$agente) {
      ...
      inner join tbestadoventa ev on ev.idestadoventa = v.refestadoventa
		...
		where v.refestadoventa in (2,3,4,5,7,8,9,10) ".$where.$cadFecha.$cadAgente."
      ...
      inner join dbventas v on v.refcotizaciones = vf.refcotizaciones and v.version = vf.version and v.refproductosaux = vf.refproductosaux
          ...
          tbtipoperiodicidad tp ON tp.idtipoperiodicidad = pv.reftipoperiodicidad
            inner join tbtipocobranza tcc on tcc.idtipocobranza = pv.reftipocobranza
              left JOIN
          dbperiodicidadventasdetalle pvd ON pvd.refperiodicidadventas = pv.idperiodicidadventa
      where v.refestadoventa = 1 and v.fechavencimientopoliza >= current_date() ".$where.$cadFecha.$cadAgente."
      GROUP BY v.idventa , cli.apellidopaterno , cli.apellidomaterno , cli.nombre , v.refproductosaux , pro.producto , ase.apellidopaterno , ase.apellidomaterno ,
      ase.nombre , aso.apellidopaterno , aso.apellidomaterno , aso.nombre , v.primaneta , v.primatotal , v.foliotys , v.fechavencimientopoliza , v.nropoliza ,
      est.estadocotizacion , v.foliointerno , c.refclientes , c.refproductos , c.refasesores , c.refasociados , c.refestadocotizaciones , c.observaciones ,
      c.fechacrea , c.fechamodi , c.usuariocrea , c.usuariomodi , c.refusuarios , tp.meses, v.refestadoventa
	...
	function traerVentasPorId($id) {
		$sql = "select idventa,refcotizaciones,refestadoventa,primaneta,primatotal,fechacrea,fechamodi,usuariocrea,usuariomodi,nropoliza,fechavencimientopoliza,foliotys,foliointerno,refproductosaux,refventas,version,refmotivorechazopoliza,observaciones,vigenciadesde,fechaemision,reftipomoneda,comisioncedida,financiamiento,gastosexpedicion,iva from dbventas where idventa =".$id;
		$res = $this->query($sql,0);
		return $res;
	}

	function traerVentasPorCotizacion($id) {
		$sql = "select idventa,refcotizaciones,refestadoventa,primaneta,primatotal,fechacrea,fechamodi,usuariocrea,usuariomodi,nropoliza,fechavencimientopoliza,foliotys,foliointerno,refproductosaux,refproductosaux,refventas,version,refmotivorechazopoliza,observaciones,vigenciadesde,fechaemision,reftipomoneda,comisioncedida,financiamiento,gastosexpedicion,iva from dbventas where refcotizaciones =".$id;
		$res = $this->query($sql,0);
		return $res;
	}

   function traerVentasPorCotizacionPaquetes($id, $refproductos) {
		$sql = "select idventa,refcotizaciones,refestadoventa,primaneta,primatotal,fechacrea,fechamodi,usuariocrea,usuariomodi,nropoliza,fechavencimientopoliza,foliotys,foliointerno,refproductosaux,refproductosaux,refventas,version,fechaemision,reftipomoneda,comisioncedida,financiamiento,gastosexpedicion,iva from dbventas where refcotizaciones =".$id." and refproductosaux =".$refproductos;
		$res = $this->query($sql,0);
		return $res;
	}
   ...
   function traerVentasPorCotizacionPaquetesUnico($id) {
	...
	function traerVentasPorIdCompleto($id) {
      ...
      cli.email, ase.email as emialasesor, v.refproductosaux, v.refventas, v.version, v.refmotivorechazopoliza, v.observaciones, v.vigenciadesde, ase.claveasesor, ase.envioalcliente, v.fechaemision , v.reftipomoneda,v.comisioncedida,v.financiamiento,v.gastosexpedicion,v.iva
		from dbventas v
		inner join dbcotizaciones c ON c.idcotizacion = v.refcotizaciones
      inner join dbclientes cli ON cli.idcliente = c.refclientes
      inner join dbasesores ase on ase.idasesor = c.refasesores
   ...
   function traerVentasPorAsesorCompletoAjax($length, $start, $busqueda,$colSort,$colSortDir,$asesor,$idcliente,$min,$max,$estado) {
      ...
      switch ($estado) {
         ...
         break;
         case 3:
            $cadEstado = ' and v.refestadoventa in (3,4,5,6,7,8,9,10) ';
         break;
      }
   ...
   function traerVentasPorAsesorCompleto($idasesor) {
      ...
      v.observaciones , v.vigenciadesde ,v.fechaemision , v.reftipomoneda,v.comisioncedida,v.financiamiento,v.gastosexpedicion,v.iva
		from dbventas v
      ...
      v.fechavencimientopoliza,pv.idperiodicidadventa,v.refproductosaux, v.refventas, v.version, v.refmotivorechazopoliza,
      v.observaciones , v.vigenciadesde ,v.fechaemision , v.reftipomoneda,v.comisioncedida,v.financiamiento,v.gastosexpedicion,v.iva
   ...
   function traerVentasPorAsesorVentaFundamental($idasesor,$idventa) {
      ...
      group by v.idventa,v.refcotizaciones,v.refestadoventa,v.primaneta,
		...
		c.refproductos,
      p.producto,
      c.tieneasegurado,ase.apellidopaterno, ase.apellidomaterno, ase.nombre,cli.apellidopaterno, cli.apellidomaterno, cli.nombre,v.refproductosaux, v.refventas, v.version, v.refmotivorechazopoliza,
      v.observaciones , v.vigenciadesde, v.fechaemision , v.reftipomoneda,v.comisioncedida,v.financiamiento,v.gastosexpedicion,v.iva
      order by v.fechavencimientopoliza desc";
		$res = $this->query($sql,0);
		return $res;
...
/* /* Fin de la Tabla: tbsolicitudpdf*/
   ...
   function copiarCotizacion($idcotizacion, $fechacrea,$fechamodi,$usuariocrea,$usuariomodi,$identico) {
      $version = 1;
      ...
      if ($identico == 1) {
         $sql = "INSERT INTO `dbcotizaciones`
                  ...
                  SELECT '',
                      ...
                      `refasesores`,
                      `refasociados`,
                      `refestadocotizaciones`,
                      `cobertura`,
                      `reasegurodirecto`,
                      'Renovación en inbursa',
                      `presentacotizacion`,
                      NULL,
                      `fecharenovacion`,
                      `fechaemitido`,
      ...
      if ((integer)$res > 0) {
         ...
         // si es identico tambien copio archivos
         if ($identico == 1) {
   ...
   function traerCotizacionesajaxPorAsesor($length, $start, $busqueda,$colSort,$colSortDir,$idasesor,$whereEstado) {
	...
	function traerCotizacionesPorIdCompleto($id) {
		$sql = "select
         ...
         pro.reftipoproductorama, c.primaobjetivototal, c.reftipomoneda, c.folioagente, c.vigenciapoliza
		from dbcotizaciones c
		inner join dbclientes cli ON cli.idcliente = c.refclientes
		inner join dbasesores ase ON ase.idasesor = c.refasesores
		inner join tbproductos pro ON pro.idproducto = c.refproductos
	...
	function traerCotizacionesPorIdCompletoV($id) {
     ...
     c.primaneta,c.primatotal, pro.reftipodocumentaciones, ase.envioalcliente, c.ot, c.articulo, c.primaobjetivototal, c.reftipomoneda, c.folioagente, c.vigenciapoliza
		from dbcotizaciones c
		inner join dbclientes cli ON cli.idcliente = c.refclientes
		inner join dbasesores ase ON ase.idasesor = c.refasesores
		inner join tbproductos pro ON pro.idproducto = c.refproductos
