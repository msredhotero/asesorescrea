...
switch ($accion) {
   ...
   case 'modificarDatosClientes':
      modificarDatosClientes($serviciosReferencias,$serviciosUsuarios);
   break;
   ...
   break;
   case 'generarRenovacionDesdeCotizacion':
      generarRenovacionDesdeCotizacion($serviciosReferencias);
   break;
   ...
   case 'traerUsuariosChat':
      traerUsuariosChat($serviciosReferencias);
   break;
      ...
      if ($existeEmail == 1) {
         ...
         if ((integer)$refclientes > 0) {
            ...
            if ($reftiporegistro == 1) {
               ...
               //dependeiendo el producto lo lanzo a un lugar u otro
               if ($refproductos == 41) {
                  ...
                  $token = $serviciosReferencias->GUID();
                  //$url = 'https://asesorescrea.com/desarrollo/crm/alogin.php?token='.$token;
                  $url = 'venta/new.php?producto='.$refproductos;
                  $resAutoLogin = $serviciosReferencias->insertarAutologin($refusuarios,$token,$url,'0');
                  ...
                  $resV['mensaje'] = 'Se genero correctamente la liga y se envio por email';
...
function insertarChat($serviciosReferencias) {
   ...
   $resCot = $serviciosReferencias->traerCotizacionesPorIdCompleto($idreferencia);
   ...
   if ((integer)$res > 0) {
      ...
      //enviar a la persona a la que le escribio un email

      $folioagente = mysql_result($resCot,0,'folioagente');

      $lblcliente = mysql_result($resCot,0,'clientesolo');

      $lblproducto = mysql_result($resCot,0,'producto');

      $asunto = 'Alerta Cotización, cotización: folio: '.$folioagente.' - Cliente: '.$lblcliente.' - Producto: '.$lblproducto.' - Asesor: '.mysql_result($resCot,0,'asesor');
...
function traerUsuariosChat($serviciosReferencias) {
   session_start();
   ...
   $chat = new chatCrea($email, $emaildestinatario,$_SESSION['idroll_sahilices'],$emailasesor,$idreferencia);

   $resCotizacion = $serviciosReferencias->traerCotizacionesPorId($idreferencia);

   $seteo = '';

   if ($emaildestinatario == '') {
      //si es un asesor, seteo el area tecnica en cuestion para iniciar el chat
      if ($_SESSION['idroll_sahilices'] == 7) {
         //determino el producto para avisar a quien corresponda, por ahora solo para los roles de areas tecnicas y sus contactos
         $refproductos = mysql_result($resCotizacion,0,'refproductos');

         $resProduto = $serviciosReferencias->traerProductosPorId($refproductos);

         $idrol = mysql_result($resProduto,0,'refgestion');

         $resUsuarios = $serviciosReferencias->traerUsuariosPorRoles($idrol);

         if (mysql_num_rows($resUsuarios)>0) {
            $seteo = mysql_result($resUsuarios,0,'email');
         } else {
            $seteo = 'ruth-arana@asesorescrea.com';
         }
      } else {
         //si es un asesor seteo el email que cargo la cotizacion
         $seteo = mysql_result($resCotizacion,0,'usuariocrea');
      }
   }
   ...
   while($mensaje = mysql_fetch_array($resultado)) {
      ...
      if (($emaildestinatario == $mensaje['email']) || ($seteo == $mensaje['email'])) {
...
function modificarVentasUnicaDocumentacionAux($serviciosReferencias) {
   ...
   $res = $serviciosReferencias->modificarVentasUnicaDocumentacion($idventa, $campo, $valor);
...
function generarRenovacionDesdeCotizacion($serviciosReferencias) {
   session_start();

   $idventa = $_POST['id'];
   $fechacrea = date('Y-m-d H:i:s');
   $fechamodi = date('Y-m-d H:i:s');
   $usuariocrea = $_SESSION['usuaid_sahilices'];
   $usuariomodi = $_SESSION['usuaid_sahilices'];
   $identico = 0;


   $resVentas = $serviciosReferencias->traerVentasPorId($idventa);

   $idcotizacion = mysql_result($resVentas,0,'refcotizaciones');

   $resCotizaciones = $serviciosReferencias->copiarCotizacion($idcotizacion, $fechacrea,$fechamodi,$usuariocrea,$usuariomodi,$identico);

   if ($resCotizaciones['error']) {
      $resV['error'] = true;
      $resV['mensaje'] = 'Se genero un error el crear la renovacion por cotizacion: '.$resCotizaciones['mensaje'];
   } else {
      $resV['error'] = false;
      $resV['mensaje'] = 'Se genero correctamente la renovacion';
      $resV['id'] = $resCotizaciones['id'];

      $resRenovacion = $serviciosReferencias->insertarRenovaciones($idventa,0,$fechacrea,$idcotizacion,$resV['id']);
...
function modificarDatosClientes($serviciosReferencias,$serviciosUsuarios) {
   $id = $_POST['id'];
...
function insertarVentasCompleto($serviciosReferencias) {
   ...
   if ((integer)$res > 0) {
      ...
      $tabla = 'dbventas';
      $operacion = 'I';
      $serviciosReferencias->insertAuditoria($tabla, $operacion,$resVenta,$usuariocrea,null,null,'1',0);
      /**** fin audi  ****/
...
function generarRenovacion($serviciosReferencias) {
   ...
   // genero la cotizacion identica
   $identico = 1;

   //copio la cotizacion anterior con todos sus archivos
   $resCopiar = $serviciosReferencias->copiarCotizacion($refcotizaciones, $fechacrea,$fechamodi,$usuariocrea,$usuariomodi,$identico);

   if ($resCopiar['error']) {
      $resV['error'] = true;
      $resV['mensaje'] = 'Se genero un problema el crear la cotizacion, verifique';
   } else {

      $refcotizacionesnueva = $resCopiar['id'];
...
function insertarVentas($serviciosReferencias) {
   ...
   */
   ...
   //die(var_dump($_POST['idrenovacion']));
   ...
   if ((integer)$res > 0) {
      ...
      if ((isset($_POST['idrenovacion'])) && ($_POST['idrenovacion'] != 0)) {
         $resRenovacion = $serviciosReferencias->modificarRenovacionesPorVenta($_POST['idrenovacion'], $res);
      }

      /**** auditoria ****/
      $tabla = 'dbventas';
      $operacion = 'I';
      $serviciosReferencias->insertAuditoria($tabla, $operacion,$res,$usuariocrea,null,null,'1',0);
      /**** fin audi  ****/
...
function insertarFirmarcontratos($serviciosReferencias) {
   ...
   $resNIPvalido = $serviciosReferencias->traerTokensPorCotizacionNIP($refcotizaciones,$nip);


   if (mysql_num_rows($resNIPvalido) > 0) {
      if (mysql_num_rows($resNIP) > 0) {
      ...
      $resV['error'] = true;
      $resV['mensaje'] = 'El NIP es incorrecto';
      $resV['tipo'] = 1;
   }
...
function eliminarVentasDefinitivo($serviciosReferencias) {
   ...
   $tabla = 'dbventas';
   $operacion = 'E';
   $serviciosReferencias->insertAuditoria($tabla, $operacion,$id,$_SESSION['usua_sahilices'],null,null,'1',0);
   /**** fin audi  ****/
...
function modificarCotizaciones($serviciosReferencias) {
   ...
   if (isset($_POST['idrenovacion'])) {
      $idrenovacion = $_POST['idrenovacion'];

      // verifico sino la cree anteriormente
   ...
   */
   ...
   if ($res == true) {
      ...
      if ($idrenovacion != 0) {
         $resVentas = $serviciosReferencias->traerVentasPorId($idrenovacion);

         if (mysql_num_rows($resVentas)>0) {
            //inserto el registro que me marca la renovacion
            $resRenovacion = $serviciosReferencias->insertarRenovaciones(mysql_result($resVentas,0,'idventa'),0,$fechamodi,mysql_result($resVentas,0,'refcotizaciones'),$id);
         }
